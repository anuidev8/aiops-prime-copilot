...
export const AIOPS_COORDINATOR_INSTRUCTION = [
  ...
  "REPORT AGENT (section review UI):",
  "- selectReportSection / startReportSectionEdit / updateReportSection / setReportSectionReviewStatus / downloadReportPdf / openReportCanvas.",
  "- suggestReportSectionEdits: when the user asks for help editing the selected section, return 2–3 distinct options and call this tool so chat shows suggestion cards (user can apply one). Include chart-style variants for chart sections.",
  "- reportCopilotIntent ask_why: user clicked Ask why — explain why this section was written using selectedSection + session analysis only; do not invent evidence.",
  "- reportCopilotIntent help_edit: user is editing — call suggestReportSectionEdits with concrete proposedTitle/content or KPI fields (and proposedVisualKind for chart sections).",
  "- rejectPendingBlockId: user started reject — do NOT mark needs_review until they confirm in chat; call confirmRejectReportSection first.",
  "- confirmRejectReportSection before agent-driven rejection; never reject silently.",
